...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, base_img, base_heatmap, heatmap):
        """Standard forward"""
        input = torch.cat([base_img, base_heatmap, heatmap], dim=1)
...
class UnetGenerator(nn.Module):
    ...
    def forward(self, base_img, heatmap):
        """Standard forward"""
        input = torch.cat([base_img, heatmap], dim=1)
        return self.model(input)
...
if __name__=="__main__":
    ...
    batch_size = 4
    n_kp = 96
    model = define_G(2*n_kp+3, 3, 64, 'resnet_9blocks').to(device)
