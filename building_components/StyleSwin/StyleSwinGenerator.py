...
from .StyleSwin_basic_layers import (EqualLinear, PixelNorm,
...
class IH_FeatureExtractor(nn.Module):
    def __init__(self, input_channels, output_size):
        super(IH_FeatureExtractor, self).__init__()

        # Load pretrained ResNet-18 model
        resnet = models.resnet18(pretrained=True)

        # Freeze the pretrained ResNet weights
        for param in resnet.parameters():
            param.requires_grad = False
        
        # Modify the first layer to fit the input channel size
        resnet.conv1 = nn.Conv2d(input_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)       

        # Extract desired layers and add classifier
        ...
        self.classifier = nn.Linear(resnet.fc.in_features, output_size)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.imFE = IH_FeatureExtractor(2*n_kp+3, style_dim)       # 6 because of base_image and depth
    ...
    def forward(
        ...
        x = torch.cat([base_image, base_heatmaps, heatmaps], dim=1)
        noise = self.imFE(x)
